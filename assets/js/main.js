...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function ensureNoextUI(){
    ...
    modal.querySelector('.noext-modal-close').addEventListener('click',()=>modal.classList.remove('open'));
  ...
  window.noextOpenModal=function(title,html){const modal=document.getElementById('noext-modal');document.getElementById('noext-modal-content').innerHTML=`<h3>${title}</h3>${html}`;modal.classList.add('open');modal.querySelector('.noext-modal-close').focus()};
  document.addEventListener('keydown',e=>{if(e.key==='Escape')document.getElementById('noext-modal')?.classList.remove('open')});
